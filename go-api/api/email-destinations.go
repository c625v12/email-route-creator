...
func GetDestinationEmails(w http.ResponseWriter, r *http.Request, httpClient *http.Client) {

	apiConfig := config.NewAPIConfig()
	url := fmt.Sprintf("%s/accounts/%s/email/routing/addresses", apiConfig.BaseURL, apiConfig.AccountId)
	...
	var apiResponse models.CloudflareDestinationEmails
	...
	currentDestinationEmails := utility.MapDestinationEmailRoutes(apiResponse.Result)
	w.Header().Set("Content-Type", "application/json")
	utility.Logger(w, "Success", http.StatusAccepted)

	if err := json.NewEncoder(w).Encode(currentDestinationEmails); err != nil {
